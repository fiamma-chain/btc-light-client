...
contract BtcTxVerifierTest is Test {
    ...
    function testVerifyTx() public {
        ...
        assertTrue(verif.verifyPayment(1, 736000, txP, 0, destScript, 25200000, false, 0, 0));

        vm.expectRevert("Not enough Bitcoin block confirmations");
        assertTrue(!verif.verifyPayment(2, 736000, txP, 0, destScript, 25200000, false, 0, 0));

        vm.expectRevert("Amount mismatch");
        assertTrue(!verif.verifyPayment(1, 736000, txP, 0, destScript, 25200001, false, 0, 0));

        vm.expectRevert("Script mismatch");
        assertTrue(!verif.verifyPayment(1, 736000, txP, 1, destScript, 25200000, false, 0, 0));

        vm.expectRevert("Block hash mismatch");
        assertTrue(!verif.verifyPayment(1, 700000, txP, 0, destScript, 25200000, false, 0, 0));
    }
    ...
    function testVerifyP2WSHTx() public {
        ...
        assertTrue(verif.verifyPayment(1, 258185, txP, 0, destScript, 100000, false, 0, 0));

        vm.expectRevert("Not enough Bitcoin block confirmations");
        assertTrue(!verif.verifyPayment(2, 258185, txP, 0, destScript, 100000, false, 0, 0));

        vm.expectRevert("Amount mismatch");
        assertTrue(!verif.verifyPayment(1, 258185, txP, 0, destScript, 100001, false, 0, 0));

        vm.expectRevert("Script mismatch");
        assertTrue(!verif.verifyPayment(1, 258185, txP, 1, destScript, 100000, false, 0, 0));

        vm.expectRevert("Block hash mismatch");
        assertTrue(!verif.verifyPayment(1, 258184, txP, 0, destScript, 100000, false, 0, 0));
    }
    ...
    function testVerifyP2TRTx2() public {
        ...
        assertTrue(verif.verifyPayment(1, 258691, txP, 0, destScript, 98220, false, 0, 0));

        vm.expectRevert("Not enough Bitcoin block confirmations");
        assertTrue(!verif.verifyPayment(2, 258691, txP, 0, destScript, 98220, false, 0, 0));

        vm.expectRevert("Amount mismatch");
        assertTrue(!verif.verifyPayment(1, 258691, txP, 0, destScript, 98221, false, 0, 0));

        vm.expectRevert("Script mismatch");
        assertTrue(!verif.verifyPayment(1, 258691, txP, 0, bytes(hex"00"), 98220, false, 0, 0));

        vm.expectRevert("Block hash mismatch");
        assertTrue(!verif.verifyPayment(1, 258690, txP, 0, destScript, 98220, false, 0, 0));
    }
    ...
    function testVerifyP2TRTx() public {
        ...
        assertTrue(verif.verifyPayment(1, 251954, txP, 0, destScript, 40526625105, false, 0, 0));

        vm.expectRevert("Not enough Bitcoin block confirmations");
        assertTrue(!verif.verifyPayment(2, 251954, txP, 0, destScript, 40526625105, false, 0, 0));

        vm.expectRevert("Amount mismatch");
        assertTrue(!verif.verifyPayment(1, 251954, txP, 0, destScript, 40526625106, false, 0, 0));

        vm.expectRevert("Script mismatch");
        assertTrue(!verif.verifyPayment(1, 251954, txP, 0, bytes(hex"00"), 40526625106, false, 0, 0));

        vm.expectRevert("Block hash mismatch");
        assertTrue(!verif.verifyPayment(1, 251952, txP, 0, destScript, 40526625105, false, 0, 0));
    }
}
